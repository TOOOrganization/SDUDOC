...
ToolManager.addHandler(new Handler("move.onMouseLeftDown", "left_down", false, MoveFactory, function(event){
  ...
  if(collide_list.length > 0) {
    if(SDUDocument.data["Dot2D"][collide_list[0]].type !== Dot2D.Type.INTERSECTION){
...
ToolManager.addHandler(new Handler("move.onMouseMove", "mousemove", false, MoveFactory, function(event){
  if(MoveFactory.getTarget()) {
    ...
    switch (SDUDocument.data["Dot2D"][MoveFactory.getTarget()].type) {
      case Dot2D.Type.FREE:
        SDUDocument.data["Dot2D"][MoveFactory.getTarget()].move(Graphics.getGridScalePoint(distance));
        break;
      case Dot2D.Type.DEPENDENT:
        SDUDocument.data["Dot2D"][MoveFactory.getTarget()].move(MouseInput.getMousePoint());
        break;
    }
    Graphics.refresh();
    return;
...
RenderManager.addRenderer(new Renderer("move.collide", 12, MoveFactory, function(ctx){
  ...
  if(collide_list.length > 0){
    if(SDUDocument.data["Dot2D"][collide_list[0]].type !== Dot2D.Type.INTERSECTION){
      SDUDocument.data["Dot2D"][collide_list[0]].renderCollide(ctx);
    }
  }
}));
// ================================================================================
